...
# 这个脚本是用来修改Manta的SV结果，用SURVIVOR合并之后的结果调整。
			...
			for SV_INFO in line[9:]:
				if SV_INFO.split(':')[-1] != 'NAN' and ',' not in SV_INFO.split(':')[-1] and SV_INFO.split(':')[-3] != 'NA':
					...
					if len(SV_INFO.split(':')[-3]) > len(SV_INFO.split(':')[-2]):
						...
						if End == Pos:
							SVLEN = str(len(SV_INFO.split(':')[-3]) - 1)
						...
						SVLEN = str(len(ALT) - 1)
						ID = 'INS.' + str(INS_Num)
						out.write(f'{Chrom}\t{Pos}\t{ID}\t{REF}\t{ALT}\t.\tPASS\tSVTYPE=INS;SVLEN={str(SVLEN)};END={str(int(Pos) + int(SVLEN))}\tGT\t{GT}\n')
						INS_Num += 1
					break
				else:
